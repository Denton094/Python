...
# Remove "and" and "or" from users input or query
if ((("and") in input_set) and (("or") in input_set)):
    input_set.remove("and")
    input_set.remove("or")
    
    # Assign  query words to separate variables
    wrd_1, wrd_2 = input_set
    # Check for query words in each quote
    print("\n query:" + wrd_1+ "," + wrd_2)

    for i,quote in enumerate(data_list):
        found_at1 = quote.find(wrd_1)
        found_at2 = quote.find(wrd_2)
        if(( found_at1 > 0) and (found_at2 > 0)):
            print("\n Performing AND search" + "[" + wrd_1+","+wrd_2+ "]", "Found at :",i +1,quote[0:])

# Remove "or" from users input or query     
elif ("or") in input_set:
    input_set.remove("or")
    # Assign  query words to separate variables
    wrd_1, wrd_2 = input_set
    # Check for query words in each quote
    ...
    for i,quote in enumerate(data_list):
        ...
        if(( found_at1 > 0) or (found_at2 > 0)):
            print("\n Performing OR search" + "[" + wrd_1+","+wrd_2+ "]", "Found at : ",i +1,quote[0:])


# Remove "and" from users input or query
elif ("and") in input_set:
    input_set.remove("and")
    ...
    for i,quote in enumerate(data_list):
        ...
        found_at2 = quote.find(wrd_2)
        if(( found_at1 > 0) and (found_at2 > 0)):
            print("\n Perfoming AND search" + "[" + wrd_1+","+ wrd_2+ "]", "Found at :",i +1,quote[0:])

dt2= datetime.now()
print("\nExecution time:", dt2.microsecond-dt1.microsecond, "ms")
